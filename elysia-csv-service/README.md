...
Production-ready CSV/Excel file processing microservice built with Bun.js and Elysia framework.
...
```bash
# Build image
docker build -t elysia-csv-service:latest .

# Run container
docker run -p 3001:3000 -p 50052:50051 elysia-csv-service:latest
```
...
|---------------------|---------|-------------|
| `SERVICE_NAME` | csv-service-elysia | Service name for OTEL |
...
docker-compose -f ../docker-compose.otel.yml up elysia-csv-service
```
